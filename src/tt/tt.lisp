 ...
 (let (ii result)
      ...
      (cond ((eq index 0) (flatten-sequences expr))
            ...
            (t (setq result expr)
               ...
               result))
)) ; END indexed-element-of


(defun match1 (pattern expr)
;``````````````````````````````
; For top-level match tracing.
; :nil and error results are also mapped to nil (under the assumption that
; the (match nil nil) case is unecessary for Eta rule trees).
;
  (let ((res (match pattern expr)))
    (if (or (equal res :nil) (equal res '***ERROR)) nil res))
) ; END match1
...
)) ; END unsymbolize-numbers-and-nil
